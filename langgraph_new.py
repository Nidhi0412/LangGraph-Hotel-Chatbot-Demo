...
import graphviz
from Insights_new import get_insights_for_langgraph
...
class Intent(str, Enum):
    ...
    INSIGHTS = "INSIGHTS"  # Add this new intent for hotelier insights
...
def classify_intents(state: GuestState) -> GuestState:
    ...
    intent_keywords = {
        ...
        Intent.INSIGHTS: [
            "insights", "analytics", "performance", "metrics", "occupancy", 
            "revenue", "booking", "forecast", "trend", "analysis", "report",
            "how is", "what is the", "show me", "tell me about", "booking pace",
            "revenue performance", "market share", "channel mix", "operational status"
        ]
    }
...
async def llm_classify_intents(state: GuestState) -> GuestState:
    ...
    try:
        ...
        messages = [
            SystemMessage(content="""You are a JSON-only response system for classifying hotel queries.
            Your task is to analyze the query and return a JSON object with the detected intents.
            
            Valid intents are: QA, ORE, CHECKIN, CHECKOUT, CONCIERGE, FEEDBACK, LOST_FOUND, INSIGHTS
            
            For INSIGHTS intent, look for:
            - Questions about hotel performance metrics
            - Requests for booking analysis
            - Revenue and occupancy trends
            - Market and channel performance
            - Operational status queries
            - Questions from hotel management perspective
            ...
            - "How is our booking pace looking for next month?"
            - "Show me insights about our revenue performance"
            - "What's our current occupancy trend?"
            - "How are we performing in the market?"
            - "Tell me about our channel mix performance"
...
async def insights_agent_node(state: GuestState) -> Dict:
    """Handle insights generation requests for hoteliers."""
    if Intent.INSIGHTS not in state["intents"]:
        return {"__merge__": {"responses": {}, "active_agents": []}}
    ...
    try:
        ...
        if any(word in question for word in ["booking", "pace", "forecast", "pickup"]):
            category = "Booking Pace & Forecasting"
        elif any(word in question for word in ["revenue", "profit", "adr", "revpar"]):
            category = "Revenue & Profitability"
        elif any(word in question for word in ["market", "channel", "segment"]):
            category = "Market & Channel Segmentation"
        elif any(word in question for word in ["operation", "housekeeping", "status"]):
            category = "Operational Status & Housekeeping"
        elif any(word in question for word in ["direct", "website", "funnel"]):
            category = "Direct-Channel Funnel & Website Performance"
        
        # Get hotel code and user ID from metadata
        hotel_code = state["metadata"].get("hotel_code", "34739")
        user_id = state["metadata"].get("user_id", "2058503")
        
        # Get insights using the imported function
        result = await get_insights_for_langgraph(
        ...
        if result["insights"]:
            # Format the insights into a response
            insights_text = "\n".join(result["insights"])
            return {
                "__merge__": {
                    "responses": {Intent.INSIGHTS: insights_text},
                    "active_agents": ["insights_agent"]
                }
            }
        else:
            return {
                "__merge__": {
                    "responses": {Intent.INSIGHTS: "I couldn't generate insights at this time."},
                    "active_agents": ["insights_agent"]
                }
            }
            
    except Exception as e:
        logger.error(f"Error in insights agent: {str(e)}")
        return {
            "__merge__": {
                "responses": {Intent.INSIGHTS: "I apologize, but I'm having trouble generating insights right now."},
                "active_agents": ["insights_agent"]
            }
        }
...
async def merge_responses(state: GuestState) -> GuestState:
    ...
    try:
        ...
        logger.info(f"Raw merged agent responses (pre-summary): {combined_response}")

        # Summarize the combined response using LLM
        try:
            llm = get_openai_client()
            prompt = ChatPromptTemplate.from_messages([
                ("system", 
                 "Given the following multiple hotel service responses (which may include agent names or section headers), "
                 "generate a single, concise, user-friendly answer for the guest. "
                 "Do not include agent names, section headers, or any technical details. "
                 "Only provide the most relevant information in a natural, conversational way."),
                ("human", combined_response)
            ])
            logger.info(f"Prompt sent to LLM for summarization: {prompt.messages}")
            summary = await (prompt | llm).ainvoke({"question": combined_response})
            summarized_response = summary.content.strip()
            logger.info(f"Final summarized response for UI: {summarized_response}")
        except Exception as e:
            logger.error(f"Error summarizing merged response: {str(e)}")
            summarized_response = combined_response  # fallback
            logger.info("Summarization failed, using merged response as fallback for UI.")

        # Make the agent summary more concise (for details, not for UI)
        agent_summary = f"\n(Generated by: {', '.join(state['active_agents'])})"
        
        # Create new state with the summarized response for UI
        new_state = GuestState(
            guest_question=state["guest_question"],
            ...
            final_response=summarized_response
        )
        
        logger.info(f"Successfully merged and summarized responses. Final response length: {len(new_state['final_response'])}")
...
def visualize_graph(graph: StateGraph, state: Optional[GuestState] = None, filename: str = "hotel_services_graph") -> None:
    """Generate and display a detailed visualization of the graph with actual flow information."""
    try:
        ...
        # Add graph attributes for better visualization
        dot.attr('node', shape='box', style='rounded,filled', fontname='Arial', fontsize='12')
        dot.attr('edge', fontname='Arial', fontsize='10')
        
        # Add title with query information if state is provided
        if state:
            title = f"Query: {state['guest_question'][:50]}..."
            if len(state['guest_question']) > 50:
                title += "..."
            dot.attr(label=title, fontsize='16', fontname='Arial Bold')
            dot.attr('graph', label=title, labelloc='t', labeljust='l', fontsize='16')
        
        # Track active nodes for highlighting
        active_nodes = set()
        if state:
            active_nodes.add("classify_intents")
            active_nodes.add("merge_responses")
            active_nodes.update(state.get("active_agents", []))
        
        # Convert all nodes to strings and create a mapping
        node_mapping = {str(node): node for node in graph.nodes}
        
        # Add nodes with enhanced styling
        for node_id, node in node_mapping.items():
            # Determine node style based on type and activity
            if node == "classify_intents":
                style = 'filled,rounded'
                fillcolor = 'lightblue' if node in active_nodes else 'lightgrey'
                label = f"{node}\nIntents: {[i.value for i in state['intents']] if state else ''}"
            elif node == "merge_responses":
                style = 'filled,rounded'
                fillcolor = 'lightgreen' if node in active_nodes else 'lightgrey'
                label = f"{node}\nResponses: {len(state['responses']) if state else 0}"
            elif "agent" in str(node):
                style = 'filled,rounded'
                fillcolor = 'lightyellow' if node in active_nodes else 'lightgrey'
                # Add agent-specific information
                if state and node in state.get("active_agents", []):
                    agent_intents = [intent for intent, responses in state.get("responses", {}).items() 
                                   if any(str(node) in agent for agent in state.get("active_agents", []))]
                    label = f"{node}\nIntents: {[i.value for i in agent_intents]}"
                else:
                    label = str(node)
            else:
                style = 'filled,rounded'
            ...
            dot.node(node_id, label, 
                    style=style,
                    fillcolor=fillcolor,
                    penwidth='2' if node in active_nodes else '1',
                    color='blue' if node in active_nodes else 'black')
            ...
            if isinstance(graph.edges, dict):
                for source, targets in graph.edges.items():
                    for target, condition in targets.items():
                        edges.append((str(source), str(target), condition))
            elif isinstance(graph.edges, set):
                # If edges is a set, create edges between all nodes
                nodes = list(graph.nodes)
                for i, source in enumerate(nodes):
                    for target in nodes[i+1:]:
                        edges.append((str(source), str(target), None))
        
        # Add edges to the graph
        for source, target, condition in edges:
            # Determine edge style based on activity
            if state and source in active_nodes and target in active_nodes:
                style = 'bold'
                color = 'blue'
                penwidth = '2'
            else:
            ...
            # Add edge with condition label if present
            if condition:
        ...
        with dot.subgraph(name='cluster_legend') as legend:
            ...
            legend.node('active_node', 'Active Node', style='filled,rounded', fillcolor='lightyellow', penwidth='2', color='blue')
            legend.node('inactive_node', 'Inactive Node', style='filled,rounded', fillcolor='lightgrey')
            legend.node('active_edge', 'Active Path', style='solid', color='blue', penwidth='2')
            legend.node('inactive_edge', 'Inactive Path', style='solid', color='grey')
        
        # Save and display the graph with timestamp
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"{filename}_{timestamp}"
        dot.render(filename, format='png', cleanup=True)
        display(Image(filename + '.png'))
        logger.info(f"Detailed graph visualization saved as {filename}.png")
        
    except Exception as e:
        logger.error(f"Error generating detailed graph visualization: {str(e)}", exc_info=True)
        raise
...
def create_guest_services_graph() -> StateGraph:
    ...
    workflow.add_node("insights_agent", create_agent_router("insights_agent", insights_agent_node))
    ...
    def route_to_next_agent(state: GuestState) -> str:
        ...
        intent_to_agent = {
            ...
            Intent.INSIGHTS: "insights_agent"
        }
    ...
    for agent in ["qa_agent", "ore_agent", "checkin_agent", "checkout_agent", 
                 "concierge_agent", "feedback_agent", "lost_found_agent", "insights_agent"]:
        workflow.add_conditional_edges(
            agent,
            route_to_next_agent,
            {
...
async def process_guest_query(
    ...
    graph_filename_prefix: str = None  # New parameter for filename prefix
) -> Dict:
    """
    ...
    Args:
        ...
        graph_filename_prefix: Optional prefix for the graph image filename
    """
    ...
    try:
        ...
        if visualize:
            try:
                prefix = graph_filename_prefix or "hotel_services_graph"
                visualize_graph(graph, filename=f"{prefix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
            except Exception as e:
                logger.warning(f"Could not generate graph visualization: {str(e)}")
        ...
        try:
            ...
            # Visualize final graph with actual flow if requested
            if visualize:
                try:
                    prefix = graph_filename_prefix or "hotel_services_graph"
                    visualize_graph(graph, final_state, filename=f"{prefix}_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
                except Exception as e:
                    logger.warning(f"Could not generate final graph visualization: {str(e)}")
    ...
    async def main():
        ...
        for query in queries:
            for metadata in test_metadata:
                ...
                # Always visualize for each query
                result = await process_guest_query(
                    query, 
                    metadata, 
                    mock_service,
                    visualize=True  # Always visualize
                )
